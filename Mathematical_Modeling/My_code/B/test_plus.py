        ...
        for j in range(cols):
            ...
            for ii in range(x_min, x_max + 1):
                for jj in range(y_min, y_max + 1):
                    ...
                    if math.sqrt((ii - i) ** 2 + (jj - j) ** 2) <= radius_matrix[ii, jj] and radius_matrix[ii, jj] != 0:  # / 37.04
...
def calculateDistance(Radius_Matrix, Activation_Matrix, Adjugate_Matrix):
    """
    求一次测线经过的点
    :param Radius_Matrix:  半径矩阵
    :param Activation_Matrix: 激活矩阵
...
def endfounction(Radius_Matrix, Activation_Matrix):
    ...
    while check_matrix_positive(Adjugate_Matrix):
        detection_line = calculateDistance(Radius_Matrix, Activation_Matrix, Adjugate_Matrix)
        update_radius_matrix(Radius_Matrix, Adjugate_Matrix)
...
a_m = create_adjugate_matrix(df)
detection_line = calculateDistance(r_m,c_m,a_m)
